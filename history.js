function renderHistory() {
    ...
    const render = async () => {
        ...
        historyList.innerHTML = workouts.length === 0 ? '<p style="text-align: center;">No workouts logged yet.</p>' : '';
    ...
    const createWorkoutArticle = (workout) => {
        ...
        const date = new Date(workout.date).toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        ...
        workout.exercises.forEach(ex => {
            ...
            if (ex.weight > 10) {
                ...
                details.textContent = `${ex.weight} kg (${weightPerSide.toFixed(2)} kg/side + 10 kg bar) × ${ex.sets} × ${ex.reps}`;
            } else {
    ...
    const exportToCSV = async () => {
        ...
        let csvContent = "data:text/csv;charset=utf-8,Date,Exercise,Weight,Sets,Reps\n";
...
`;
        ...
        link.setAttribute("download", "training_log_export.csv");
    ...
    const importFromCSV = () => {
        ...
        reader.onload = async (event) => {
            ...
            const newWorkouts = {};
            const allExerciseNames = new Set((await db.exercises.toArray()).map(e => e.name));
